...
namespace CoreDemo.Controllers
{
    public class EmployeeController : Controller
    {
        public async Task<IActionResult> Index()
        {
            ...
            var jsonnString = await responseMessage.Content.ReadAsStringAsync();
            var values = JsonConvert.DeserializeObject<List<Class1>>(jsonnString);
        ...
        public async Task<IActionResult> EditEmployee(int id)
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonEmplyee = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<Class1>(jsonEmplyee);
                return View(values);
            }
            return RedirectToAction("Index");
        }
